...
def test_convert_id_columns_to_int():
    # Create test data
    X = pd.DataFrame({"id": ["A", "B", "C"], "value": [1, 2, 3]})

    eval_X1 = pd.DataFrame({"id": ["D", "E", "A"], "value": [1, 4, 5]})

    eval_X2 = pd.DataFrame({"id": ["E", "C", "F"], "value": [3, 5, 6]})

    eval_set = [(eval_X1, pd.Series([1, 0, 1])), (eval_X2, pd.Series([0, 1, 0]))]
    ...
    # Call the method
    result_X, result_eval_set = enricher._convert_id_columns_to_int(X, eval_set)

    # Verify results
    # Check that all id columns are converted to integers
    assert result_X["id"].dtype == np.int8
    assert result_eval_set[0][0]["id"].dtype == np.int8
    assert result_eval_set[1][0]["id"].dtype == np.int8

    # Check that the encoding is consistent across all datasets
    # Get unique values and their encodings
    all_ids = pd.concat([result_X["id"], result_eval_set[0][0]["id"], result_eval_set[1][0]["id"]]).unique()
    ...
    id_mapping = {}
    for df in [result_X, result_eval_set[0][0], result_eval_set[1][0]]:
    ...
    assert result_X["value"].equals(X["value"])
    assert result_eval_set[0][0]["value"].equals(eval_X1["value"])
    assert result_eval_set[1][0]["value"].equals(eval_X2["value"])
...
def test_convert_id_columns_to_int_no_id_columns():
    ...
    assert result_X.equals(X)
    assert result_eval_set[0][0].equals(eval_X1)
    assert result_eval_set[1][0].equals(eval_X2)
