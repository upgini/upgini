...
def test_trend_coef():
    ...
    assert_series_equal(feature.calculate(df), expected_res, check_dtype=False, atol=1e-6, rtol=1e-6)
...
def test_trend_coef_polyfit_fails():
    ...
    assert_series_equal(feature_constant.calculate(df_constant).astype(float), expected_res_constant)
