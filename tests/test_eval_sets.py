...
def test_calculate_metrics_same_objects_with_oot_dataframe_skips_transform(requests_mock: Mocker):
    ...
    is_same, same_X, same_y, same_eval = enricher._is_input_same_as_fit(train_X, train_y, [(eval_X, eval_y), oot_X])
    assert is_same is True
...
def test_metrics_cache_key_isolates_outliers_and_exclude(requests_mock: Mocker):
    ...
    key_exclude_other_order = enricher._get_metrics_cache_key(X, y, None, exclude_features_sources=["ads_b", "ads_a"])
    key_exclude_sorted = enricher._get_metrics_cache_key(X, y, None, exclude_features_sources=["ads_a", "ads_b"])
...
def test_metrics_cache_derives_multi_dimension_from_fullest_entry(requests_mock: Mocker):
    ...
    oot_y = pd.Series([np.nan])
    eval_set = [
        (eval_X.drop(columns=[ENTITY_SYSTEM_RECORD_ID]), eval_y),
        (oot_X.drop(columns=[ENTITY_SYSTEM_RECORD_ID]), oot_y),
    ]
