...
def test_balance_undersampling_binary():
    ...
    balanced_df = balance_undersample(
        df, TARGET, ModelTaskType.BINARY, 42, binary_min_sample_threshold=2
    )
    ...
    balanced_df = balance_undersample(
        df, TARGET, ModelTaskType.BINARY, 42, binary_min_sample_threshold=3
    )
    ...
    balanced_df = balance_undersample(
        df, "target", ModelTaskType.BINARY, 42, binary_min_sample_threshold=4
    )
...
def test_balance_undersaampling_multiclass():
    ...
    balanced_df = balance_undersample(
        df, TARGET, ModelTaskType.MULTICLASS, 42, multiclass_min_sample_threshold=10
    )
    ...
    balanced_df = balance_undersample(
        df, TARGET, ModelTaskType.MULTICLASS, 42, multiclass_min_sample_threshold=5
    )
    # a - 1, b - 2, c - 4
    expected_df = pd.DataFrame(
        {SYSTEM_RECORD_ID: [1, 2, 3, 4, 5, 6, 8], TARGET: ["a", "b", "c", "c", "c", "b", "c"]}
    )
