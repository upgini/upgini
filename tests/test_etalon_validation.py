...
def test_one_hot_encoding_validation():
    df = pd.DataFrame(
        {
            ...
            "text_feature_sd98f7": ["text_" + str(n) for n in np.random.rand(1000)],
        }
    )
    ...
    warnings_counter = WarningCounter()
    features_to_drop = FeaturesValidator().validate(
        df, features_columns, ["text_feature"], {"text_feature": "text_feature_sd98f7"}, warnings_counter
    )
